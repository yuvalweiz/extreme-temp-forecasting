...
def pats(model, season, reg):
    ...
    if model == "ConvNeXtTiny-TFT-NN":
        return [f"{E}/Center_tune/nnfs2std_a1b0__*"] if (season=="summer" and reg=="Center") else (
               [f"{E}/{reg}_stable/nnfFIN_std__*"] if season=="summer" else [f"{E}/{reg}_cold/nnfwin__*"])
    ...
    if model == "Tab-LSTM":
        return [f"{E}/{reg}_stable/stl__*"] if season=="summer" else [f"{E}/{reg}_cold/{s}_cstl__*"]
    if model == "Tab-TFT":
        if season == "summer":
    ...
    for season in ["summer", "winter"]:
        for reg in ["Center", "Northwest", "Negev"]:
            ...
            rows.append(dict(model=model, season=season, region=reg, n=len(sm),
                             ens=ens, seed_mean=sm.mean() if len(sm) else np.nan,
